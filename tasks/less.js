...
module.exports = function (grunt) {
      ...
      function (f, nextFileObj) {
        ...
        async.concatSeries(
          ...
          function (file, next) {
            ...
            compileLess(file, destFile, options).then(
              function (output) {
                ...
                if (options.sourceMap && !options.sourceMapFileInline) {
                  ...
                  if (typeof sourceMapFilename === 'function') {
                    try {
                      ...
                      sourceMapFilename = destFile + '.map';
                      grunt.fail.warn(
                        wrapError(e, 'Generating sourceMapFilename failed.')
                      );
                    }
                  }
                  grunt.file.write(sourceMapFilename, output.map);
                  grunt.verbose.writeln(
                    'File ' + chalk.cyan(sourceMapFilename) + ' created.'
                  );
                  tally.maps++;
                }
          ...
          function () {
            if (compiled.length < 1) {
              grunt.log.warn(
                'Destination ' +
                  chalk.cyan(destFile) +
                  ' not written because compiled files were empty.'
              );
            } else {
              var allCss = compiled.join(
                options.compress
                  ? ''
                  : grunt.util.normalizelf(grunt.util.linefeed)
              );
              ...
              grunt.verbose.writeln(
                'File ' + chalk.cyan(destFile) + ' created'
              );
              tally.sheets++;
            }
            nextFileObj();
      ...
      function () {
        if (tally.sheets) {
          grunt.log.ok(
            tally.sheets +
              ' ' +
              grunt.util.pluralize(tally.sheets, 'stylesheet/stylesheets') +
              ' created.'
          );
        }
        ...
        if (tally.maps) {
          grunt.log.ok(
            tally.maps +
              ' ' +
              grunt.util.pluralize(tally.maps, 'sourcemap/sourcemaps') +
              ' created.'
          );
        }
  ...
  var compileLess = function (srcFile, destFile, options) {
    ...
    if (
      ...
      options.sourceMap &&
      !options.sourceMapFileInline &&
      typeof options.sourceMapFilename === 'function'
    ) {
      try {
    ...
    return less.render(srcCode, options).catch(function (err) {
